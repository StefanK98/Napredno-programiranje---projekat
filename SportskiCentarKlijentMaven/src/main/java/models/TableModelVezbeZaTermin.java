...
public class TableModelVezbeZaTermin extends AbstractTableModel implements Runnable{
    ...
    private List<VezbaZaTermin> listaVezbi;
    ...
    public Object getValueAt(int i, int i1) {
        VezbaZaTermin vzt = listaVezbi.get(i);
    ...
    private void osveziTabelu() {
        try{
            listaVezbi = ClientController.getInstance().getAllVezbaZaTermin();
            if(!parametar.equals("")){
                List<VezbaZaTermin> novaLista=new ArrayList<>();
                for(VezbaZaTermin vzt: listaVezbi)
                    if(vzt.getVezba().toLowerCase().equals(parametar.toLowerCase()))
                        novaLista.add(vzt);
                listaVezbi=novaLista;
            }else{
